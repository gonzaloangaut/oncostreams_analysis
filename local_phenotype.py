...
def calculate_local_phenotype(num_cells, max_step, dens, step, rng_seed, box_length):
...
os.makedirs("graphs/local_cell_phenotype", exist_ok=True)
...
for dens in density:
    ...
    for length in box_lengths:
        mean_aspect_ratio, fraction_elongated, fraction_round, fraction_binary, steps = calculate_local_phenotype(
        ...
        plt.title(f"Phenotype and aspect ratio for ρ={dens} and box length={int(length)}")
        plt.savefig(f"graphs/local_cell_phenotype/phenotype_aspect_ratio_rho={dens:.2f}_box_length={int(length)}.jpg", dpi=600)
        plt.close()
