    ...
    for tic in range(0, max_step + 1, step):
        for seed in rng_seed:
            ...
            if os.path.exists(dat_actual):
                ...
                output_file = f"data/N={num_cells:_}/{dens_folder}/dat_clusters/clusters_culture_initial_number_of_cells={num_cells}_density={dens}_force=Anisotropic_Grosmann_k=3.33_gamma=3_With_Noise_eta=0.033_With_Shrinking_rng_seed={seed}_step={tic:05}.dat"
...
def save_distribution_last_step(num_cells, dens, rng_seed, last_step):
    ...
    dens_folder = f"{dens:.3f}".replace(".", "_")
    output_dir = f"data/N={num_cells:_}/{dens_folder}/cluster_distributions_final"
