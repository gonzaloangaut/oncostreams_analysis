...
def calculate_final_phenotype(num_cells, max_step, dens, step, rng_seed):
    ...
    for seed in rng_seed:
        dat_actual = f"{dens_folder}/dat_phenotype/phenotype_culture_initial_number_of_cells={num_cells}_density={dens}_force=Anisotropic_Grosmann_k=3.33_gamma=3_With_Noise_eta=0.033_With_Shrinking_rng_seed={seed}_step={ultimo_step:05}.dat"
...
os.makedirs("graphs/steady_state_cell_phenotype", exist_ok=True)
...
for dens in density:
    result = calculate_final_phenotype(nc, max_step + 1, dens, step, rng_seed)
    mean = result["mean"]
    std = result["std"]
...
plt.title("Steady State Cell Phenotypes (Mean ± Std Error)")
plt.savefig("graphs/steady_state_cell_phenotype/steady_state_phenotype.jpg", dpi=600)
plt.close()
...
plt.ylabel("Variance")
plt.title("Variance of Cell Phenotypes vs Density")
plt.legend()
plt.tight_layout()
plt.savefig("graphs/steady_state_cell_phenotype/steady_state_phenotype_variance.jpg", dpi=600)
plt.close()
